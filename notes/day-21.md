...
The repeatable change check works on a temporary copy of the database. It adds £0.01 to one synthetic monthly plan, runs the snapshot again and expects five versions in total: four current rows and one closed row. The normal local database and committed synthetic inputs are not changed. After correcting the key, GitHub Actions confirmed five total versions, four current rows and one closed row.
